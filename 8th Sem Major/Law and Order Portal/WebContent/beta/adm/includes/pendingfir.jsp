...
<%
	int uid = (Integer) session.getAttribute("pid");// (String) session.getAttribute("citizenid");
		...
		if (uid != 0) {
			...
			Response resp = police.retrieveComplainFIR(dept_id, vps_id);// out.println(resp.getEntity());
			try {
	...
	<table>
		...
		<tr>
			...
			<td><div class="buttonType ">
					<a href="./includes/registerfir.jsp"
						data-content="citizenId=<%=citizen_id.get(i)%>&complaintId=<%=complaint_id.get(i)%>&id=id<%=i%>&ref=pendingfir"
						id="id<%=i%>" class="custompopup">Register FIR</a>
				</div>
			</td>
		</tr>
		<%
