...
<%
	int uid = (Integer) session.getAttribute("pid");// (String) session.getAttribute("citizenid");
		...
		if (uid != 0) {
			...
			Response resp = police.retrieveallComplain(vps_id);// out.println(resp.getEntity());
			try {
		...
		<tr>
			...
			<td><div class="buttonType ">
					<a href="./includes/showcomplaint.jsp"
						data-content="citizenId=<%=citizen_id.get(i)%>&complaintId=<%=complaint_id.get(i)%>&id=id<%=i%>&ref=new"
