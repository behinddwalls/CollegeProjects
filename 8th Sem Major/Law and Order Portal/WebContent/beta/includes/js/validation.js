		...
		.ready(
				function() {
					...
					registerForm.submit(function(e) {
						...
						// following the link
						e.stopPropagation();
						if (validateFName() & validateEmail() & validateDOB()
								& validateAddress_block() & validateGender()
								& validatepincode() & validatePass1()
								& validatePass2() & validateFile()
								& validateMobile()) {
							alert("hello2");
							var data_content = registerForm.serialize();
							$.ajax({
								url : $(this).attr('href'),
								cache : false,
								data : data_content,
					...
					form
							.submit(function(e) {
								...
								if (validateFName() & validateEmail()
										...
										& validateGender() & validatepincode()
										& validatePass1() & validateMobile()) {
									alert("hello1");
									var data_content = form.serialize();
									$
											.ajax({
												url : "./site/submit/submit_update_profile.jsp",
												cache : false,
												data : data_content,
												success : function(data) {
													alert("success" + data);
					...
					$("#picForm #send").click(function(e) {
						...
						if (validatePic()) {
							...
							$("#preview").html('');
							$("#preview").html('<p><img src="./includes/image/loader.gif" alt="Uploading...." width="50px" height="20px"/></p>');
							$("#picForm").ajaxForm({
								success : function(data){
									alert(data);
									...
									$.ajax({
										url:"./site/submit/submit_profile_pic.jsp",
										type:"GET",
										data:"url="+filename,
										success: function(data)
										{    
